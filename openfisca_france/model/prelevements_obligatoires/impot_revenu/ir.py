...
class age(Variable):
    ...
    def set_input(holder, period, array):
        ...
        naissance = (datetime64(period.first_month.start).astype('datetime64[M]') - (array * 12).astype('timedelta64[M]')).astype('datetime64[D]')
        holder.simulation.set_input('date_naissance', period, naissance)
...
class rpns_individu(Variable):
    ...
    def formula(individu, period, parameters):
        ...
        cbnc_assc = individu('f5qm', period)      # noqa F841
        mncn_impo = individu('mncn_impo', period)
        cncn_bene = individu('cncn_bene', period)
        cncn_defi = individu('cncn_defi', period)
